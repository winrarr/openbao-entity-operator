...
.PHONY: generate
generate: controller-gen crd-ref-docs
...
kind-install-cni: kind-ensure-cni
	@case "$(KIND_CNI)" in \
		default) ;; \
...
kind-load-openbao-image: kind-ensure-cni
...
kind-install-openbao: kind-install-cni kind-load-openbao-image
...
kind-load-image: kind-create docker-build
...
kind-deploy-e2e: kind-deploy
	...
	$(MAKE) HELM_INSTALL_ARGS="--set-string watchNamespaces[0]=$(E2E_TEST_NAMESPACE) --set-string watchNamespaces[1]=$(E2E_TENANT_B_NAMESPACE)" HELM_ARGS="--kube-context=kind-$(KIND_CLUSTER)" helm-install
...
kind-e2e-resilience: kind-deploy ## Run focused operator recovery scenarios against a persistent TLS fixture in Kind.
	...
	$(MAKE) HELM_INSTALL_ARGS="--set-string watchNamespaces[0]=$(PERSISTENT_E2E_NAMESPACE)" HELM_ARGS="--kube-context=kind-$(KIND_CLUSTER)" helm-install
	...
	$(MAKE) PERSISTENT_OPENBAO_NAMESPACE="$(PERSISTENT_OPENBAO_NAMESPACE)" PERSISTENT_E2E_NAMESPACE="$(PERSISTENT_E2E_NAMESPACE)" kind-e2e-resilience-clean
