...
OpenBao Entity Operator lets a platform team declare OpenBao ACL policies, identity entities, and groups as Kubernetes resources. It manages them through an authenticated OpenBao connection and exposes readiness, external identity, membership, policy version/hash, and reconciliation failures through Kubernetes status.
