...
configurable cross-namespace reference policy. It now provides the narrower
`watchNamespaces` deployment scope. It also does not provide Infisical Entity
Operator-style organization and project CRDs with organization IDs validated on
dependent resources.
...
The remaining connection-ownership and tenant resource-kind decisions are
