...
| `make openapi-check` | The checked-in OpenBao reference is valid and contains the entity endpoints used here | The reference is version-specific and dynamic |
...
| `make kind-e2e` | Runs live connection, entity lifecycle, drift, adoption, conflict, orphan, and delete scenarios against OpenBao in Kind | Uses a single in-memory OpenBao dev server |
