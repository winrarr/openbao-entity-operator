...
| `make test` | Client HTTP behavior, Kubernetes Auth login/renew/re-login, and reconciliation transitions pass unit tests | Does not exercise a live Kubernetes API server or OpenBao |
...
| `make kind-e2e` | Installs the operator from the Helm chart and runs live Kubernetes Auth connection, entity, alias, group, and membership lifecycle, drift, adoption, conflict, orphan, delete, missing-cleanup-dependency, and namespace-isolation scenarios against OpenBao in Kind | Uses a single in-memory OpenBao dev server; the root token only bootstraps OpenBao and configures the test auth role |
